...
describe('Vacancy Management Use Cases', () => {
  ...
  describe('GetVacanciesUseCase', () => {
    ...
    it('should return paginated vacancies', async () => {
      const mockResult = {
        data: [
          { id: 1, title: 'Software Engineer', companyName: 'TechCorp', role: 'Engineer', applicationCount: 5, createdAt: new Date() },
        ],
        total: 1,
        page: 1,
        pageSize: 10,
      ...
      expect(mockRepo.getVacancies).toHaveBeenCalledWith({ page: 1, pageSize: 10 });
      expect(result.data).toHaveLength(1);
      expect(result.data[0].title).toBe('Software Engineer');
    });

    it('should filter by search term', async () => {
      mockRepo.getVacancies.mockResolvedValue({ data: [], total: 0, page: 1, pageSize: 10, totalPages: 0 });

      await useCase.execute({ search: 'engineer' });

      expect(mockRepo.getVacancies).toHaveBeenCalledWith({ search: 'engineer' });
    });
  ...
  describe('UpdateVacancyUseCase', () => {
    ...
    it('should update vacancy successfully', async () => {
      ...
      expect(mockRepo.updateVacancy).toHaveBeenCalledWith(1, { title: 'New Title' });
    });

    it('should throw NotFoundException when vacancy not found', async () => {
      mockRepo.getVacancyById.mockResolvedValue(null);

      await expect(useCase.execute(999, { title: 'New Title' })).rejects.toThrow(NotFoundException);
  ...
  describe('DeleteVacancyUseCase', () => {
    ...
    it('should delete vacancy successfully', async () => {
      mockRepo.getVacancyById.mockResolvedValue({ id: 1, title: 'Test Vacancy' });
    ...
    it('should throw NotFoundException when vacancy not found', async () => {
      ...
      await expect(useCase.execute(999, 'admin-1')).rejects.toThrow(NotFoundException);
  ...
  describe('DeleteAllVacanciesUseCase', () => {
    ...
    beforeEach(async () => {
      ...
      useCase = module.get<DeleteAllVacanciesUseCase>(DeleteAllVacanciesUseCase);
    });
