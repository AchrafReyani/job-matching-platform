...
import { AuthenticatedRequest, getUserId } from '../../common/interfaces/authenticated-request.interface';
...
export class NewsController {
  ...
  @Get('admin')
  async getNewsAdmin(@Request() req: AuthenticatedRequest, @Query() query: NewsQueryDto) {
  ...
  @Post()
  async createNews(@Request() req: AuthenticatedRequest, @Body() dto: CreateNewsDto) {
    this.requireAdmin(req);
  ...
  async getNewsForUser(
    ...
    return this.getNewsForUserUseCase.execute(userId, userRole, page || 1, limit || 10);
  }
