...
describe('MarkNewsReadUseCase', () => {
  ...
  it('should throw NotFoundException if news does not exist', async () => {
    ...
    await expect(useCase.execute(999, 'user-1', 'JOB_SEEKER')).rejects.toThrow(NotFoundException);
  ...
  it('should throw NotFoundException if news is not published', async () => {
    ...
    await expect(useCase.execute(1, 'user-1', 'JOB_SEEKER')).rejects.toThrow(NotFoundException);
    expect(mockRepo.markAsRead).not.toHaveBeenCalled();
  });

  it('should mark news as read for JOB_SEEKER with ALL audience', async () => {
    mockRepo.findById.mockResolvedValue(mockNews);
    mockRepo.markAsRead.mockResolvedValue({ id: 1, newsId: 1, userId: 'user-1', readAt: new Date() });
  ...
  it('should mark news as read for COMPANY with ALL audience', async () => {
    mockRepo.findById.mockResolvedValue(mockNews);
    mockRepo.markAsRead.mockResolvedValue({ id: 1, newsId: 1, userId: 'company-1', readAt: new Date() });
  ...
  it('should throw ForbiddenException for company trying to read job seeker only news', async () => {
    ...
    await expect(useCase.execute(1, 'company-1', 'COMPANY')).rejects.toThrow(ForbiddenException);
    await expect(useCase.execute(1, 'company-1', 'COMPANY')).rejects.toThrow('You do not have access to this news');
    expect(mockRepo.markAsRead).not.toHaveBeenCalled();
  });
  ...
  it('should throw ForbiddenException for job seeker trying to read company only news', async () => {
    ...
    await expect(useCase.execute(1, 'user-1', 'JOB_SEEKER')).rejects.toThrow(ForbiddenException);
    expect(mockRepo.markAsRead).not.toHaveBeenCalled();
  });
