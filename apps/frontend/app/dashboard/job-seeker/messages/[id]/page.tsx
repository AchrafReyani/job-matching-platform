...
export default function JobSeekerChatPage() {
  ...
  useEffect(() => {
    const token = localStorage.getItem('token'); // Or getToken()
    if (token) {
      try {
  ...
  const companyName = application?.vacancy.company?.companyName;
  const companyUserId = application?.vacancy.company?.userId;
